...
def save_loss_plot(iteration, loss, title, dir_name="lossCurve"):
    dir_path = constant.cache_dir + dir_name + "/"
    util.create_dir(dir_path, False)
    plt.figure(title)  # 创建图表1
...
def save_accurate_plot(iteration, acc, title, dir_name="accurateCurve"):
    dir_path = constant.cache_dir + dir_name + "/"
    util.create_dir(dir_path, False)
    plt.figure(title)  # 创建图表1
    plt.title(title)  # give plot a title
